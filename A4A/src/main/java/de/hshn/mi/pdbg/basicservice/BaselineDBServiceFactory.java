...
public class BaselineDBServiceFactory {
    ...
    public static BasicDBService createBaselineDBService() {
        try {
			//Use "jdbc:postgresql://postgres/pdbg-baseline", "postgres", "postgres" for jdbc url, login and password in CI/CD context
            return PostgresSQLBaselineDBFactory.createBaselineDBService("jdbc:postgresql:pdbg-baseline", "postgres", "postgres");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
