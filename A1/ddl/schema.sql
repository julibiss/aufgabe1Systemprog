...
CREATE TABLE Physician(
    ID int PRIMARY KEY,
    FOREIGN KEY Supervisor int REFERENCES Physician(ID) ON DELETE RESTRICT,
    FOREIGN KEY W_ID int REFERENCES Ward(ID) ON DELETE RESTRICT,
    FOREIGN KEY D_ID int REFERENCES Department(ID) ON DELETE RESTRICT,
    firstname varchar (50) NOT NULL,
    lastname varchar (30) NOT NULL,
    dateOfBirth Date NOT NULL,
...
CREATE TABLE Nurse(
    ID int PRIMARY KEY,
    FOREIGN KEY Supervisor int REFERENCES Nurse(ID) ON DELETE RESTRICT,
    FOREIGN KEY W_ID int REFERENCES Ward(ID) ON DELETE RESTRICT,
...
CREATE TABLE Examination(
    ID int PRIMARY KEY,
    FOREIGN KEY E_ID int REFERENCES Examination(ID) ON DELETE RESTRICT,
...
CREATE TABLE Finding(
    ID int PRIMARY KEY,
    FOREIGN KEY PH_ID int REFERENCES Physician(ID) ON DELETE RESTRICT,
    FOREIGN KEY P_ID int REFERENCES Patient(ID) ON DELETE RESTRICT,
...
CREATE TABLE ExaminationResult(
    ID int PRIMARY KEY,
    FOREIGN KEY E_ID int REFERENCES Examination(ID) ON DELETE CASCADE,
    FOREIGN KEY F_ID int REFERENCES Finding(ID) ON DELETE RESTRICT,
